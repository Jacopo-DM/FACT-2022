...
def preprocess_gray_image(img, use_cuda=1, require_grad = False):
    mean = 0.456
    stds = 0.225

    preprocessed_img = img.copy()
    preprocessed_img = preprocessed_img  - mean
...
def Get_blurred_img(input_img, img_label, model, resize_shape=(224, 224), Gaussian_param = [51, 50], Median_param = 11, blur_type= 'Gaussian', use_cuda = 1):
    ...
    if img.shape==(260,260):
        img_torch = preprocess_gray_image(img, use_cuda, require_grad = False)
        blurred_img_torch = preprocess_gray_image(blurred_img, use_cuda, require_grad = False)
    else:
    ...
    #img_torch = img_torch.reshape(img_torch.shape[1],img_torch.shape[0],img_torch.shape[2],img_torch.shape[3])
    #blurred_img_torch = blurred_img_torch.reshape(blurred_img_torch.shape[1],blurred_img_torch.shape[0],blurred_img_torch.shape[2],blurred_img_torch.shape[3])
    ...
    #print(img_label)
    #print(blurred_output)
    #print(blurred_output[0, img_label])
...
def Integrated_Mask(img, blurred_img, model, category, max_iterations = 15, integ_iter = 20,
    ...
    if img.data.shape == (1,260,260):
        resize_wh = (img.data.shape[1], img.data.shape[2])
    else:

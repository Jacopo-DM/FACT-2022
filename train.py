...
import os
os.chdir(r"C:\Users\Bram\Downloads\scouter-master\scouter-master")
...
def main(args):
    ...
    data_loader_test = DataLoaderX(dataset_test, args.batch_size, sampler=sampler_val, num_workers=args.num_workers)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        train_one_epoch(model, data_loader_train, optimizer, device, record, epoch,testing=False)
        lr_scheduler.step()
        if args.output_dir:
        ...
        evaluate(model, data_loader_val, device, record, epoch,testing=False)
        log.print_metric()
    evaluate(model, data_loader_test, device, record, epoch, testing=True)
    log.print_metric()

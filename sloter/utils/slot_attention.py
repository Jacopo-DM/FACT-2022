...
class SlotAttention(nn.Module):
    ...
    def forward(self, inputs, inputs_x):
        ...
        for _ in range(self.iters):
            ...
            #insertion = CausalMetric(self, 'ins', 224 * 8, substrate_fn=blur)
            #deletion = CausalMetric(self, 'del', 224 * 8, substrate_fn=torch.zeros_like)
            #h = deletion.single_run(torch.from_numpy(slots_prev.astype('float32')), slots_prev, 100)
            #print(insertion)
        ...
        if self.vis:
            ...
            slots_vis=slots_vis.reshape(b,10,9,9)

...
def test(args, model, device, img, image, label, vis_id):
    ...
    for id in range(args.num_classes):
        ...
        d = deletion.single_run(torch.from_numpy(np.array(torch.unsqueeze(image, dim=0).cpu())), np.array(heatmap_on_image))
        i = insertion.single_run(torch.from_numpy(np.array(torch.unsqueeze(image, dim=0).cpu())), np.array(heatmap_on_image))
        print(np.mean(d))
        print(np.mean(i))
